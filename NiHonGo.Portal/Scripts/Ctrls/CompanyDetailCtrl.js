...
function Search(id, index, count) {
    $("#joblist").empty();
    $("#pagination").empty();

    $.ajax({
        type: 'post',
        url: '/Company/GetJobList/',
        data: {
            id: id,
            index: index,
            count: count
        ...
        success: function (data) {
            ...
            if (data.Count > 0) {
                ...
                for (i = firstPage; i < lastPage ; i++) {
                    page += '<li id="btnPage' + i + '" name="btnPage' + i + '"><a  role="button" OnClick="Search(' + id + ',' + i + ',' + count + ')">' + (i + 1) + '</a></li>'
                }
